...
export default function LateNightEats() {
  ...
  const show = () => {
    setOpen(true);
    trackEvent("late_eats_open", { count: tonight.length, day });
  };

  return (
    <>
      <button className="late-eats-launcher" type="button" onClick={show} aria-haspopup="dialog">
        <span className="late-eats-launcher-mark">EAT</span>
        <span><strong>Late Night Eats</strong><small>{tonight.length} open-late picks</small></span>
      </button>
        ...
        <div className="late-eats-overlay" role="presentation" onMouseDown={() => setOpen(false)}>
          ...
          >
            <div className="late-eats-panel-head">
              <div>
                <p>LATE NIGHT EATS · TONIGHT</p>
                <h2 id="late-eats-title">Okay, but where are we eating?</h2>
                <span>Current late-closing options with the source attached. Sorted by who keeps feeding people the longest.</span>
              </div>
